...
for ep in range(3):
    ...
    for it in range(int(len(X_all)/N)):
        ...
        savemat('Latent.mat',{'latent_x':latent_x.cpu().detach().numpy(), 'latent_x2':latent_x2.cpu().detach().numpy()})
        sigma = 0.1#(N)**(-1/(4+(1)))
        alpha = 1.01#1.01
        ...
        if k%120 == 0:
            ...
            if show_id%2 == 0:            
                ...
                savemat('current_results.mat',{'current_results':save_show})
                savemat('current_loss.mat',{'current_loss':current_loss})
